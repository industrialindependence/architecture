...
**Active poll.** A method by which the secure edge gateway acquires data from devices inside the zone: the gateway opens a session with the device — typically over a control-network protocol the device speaks (Modbus, EtherNet/IP, OPC UA, S7, etc.) — and reads tags, registers, or state on a configured cadence. Active poll requires the device to permit it and the operator to authorize it (credentials, allowed scopes). Where active poll is unavailable (the device does not support it, refuses, or cannot be safely interrupted), *witness* (passive observation) is the floor. Both feed the decentralized historian.
...
**Consumer.** The recipient of information published by a secure edge gateway. Two kinds, and only two: (1) **another gateway at broader scope** — the fractal case, where a zone gateway publishes to a plant gateway, a plant gateway publishes to a site gateway, and so on; (2) **an authorized external system** — an enterprise application, a BI/analytics lake, an AI/ML platform, a regulator-facing dashboard, a partner integration. Both kinds connect to the gateway's outbound interface (the outside subscriber in the two-box realization). Neither reaches back into the zone — the gateway is the only access in or out. The operator authorizes each consumer explicitly via the contract catalog; uncontracted consumers do not exist on the wire.
...
**Witness.** The secure edge gateway's passive observation role on the ACS substrate. The ACS-facing NIC is RX-only (no IP stack transmit, no listeners); it observes traffic via mirror port, network tap, or equivalent passive capture. Witness produces records from what it sees on the wire — protocol headers, packet flows, control-network conversations, broadcast traffic. Unlike *active poll*, witness requires no cooperation from the devices in the zone and cannot be refused. **Witness is the floor**: it is always available and is what makes brownfield deployment viable on substrates where active poll is impossible or unsafe. Active poll is added on top where the device permits and the operator authorizes. Both feed the decentralized historian. Role identifier: `ot.acs.witness`.
