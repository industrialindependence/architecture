...
2. **Minimum runtime surface.** Every listener, daemon, and outbound connection exists because it was designed and engineered for a specific operational purpose. There is no default-on service. There is no convenience listener. There is no runtime debug interface. The OS image is built so disabled services cannot be turned on at runtime — only by rebuilding the image. *Complexity is the enemy of Reliability* (Philosophy #03) is the principle this invariant operationalizes.
3. **ACS-facing observation interface is passive.** No IP stack TX, no listeners ever, on the ACS observation NIC (mirror / SPAN / TAP). Zone IP services (DHCP, DNS, NTP, file — see `netservices`) run on a separate active interface scoped to the cell network only, never routed beyond. The observation channel and the services channel can share a physical NIC via VLAN or sub-interface; the architectural distinction is functional, not physical.
...
| Zone services interface | DHCP, DNS, NTP, SMB / NFS, optional in-cell PKI — listeners scoped to the cell network | Standard RFC-compliant service responses, scoped to the cell network only, never routed beyond | Active participant on the cell. May share a physical NIC with the ACS observation NIC via VLAN or sub-interface; the distinction from the observation NIC is functional, not physical. |
...
**Role addressing.** Architectural roles are named with **leaf-first, hierarchical, hyphenated labels** — the same direction DNS resolves and the same direction ISA-95 hierarchies are conventionally written. In the spec, each role is referred to by its **shortname**: a single label such as `witness`, `lake`, `bus`, `publish`, `cfg`. At deployment time, the shortname is the leftmost segment of a fully-qualified hierarchical name that extends rightward through the gateway's identity and its placement in the operator's ISA-95 hierarchy:

    <role>.<gateway>.<work-unit>.<work-center>.<area>.<site>.local

Example, for a gateway named `seg01` at the head of `Palletizer.Packout.Zone4.Milwaukee`:
...
The convention is *leaf-first*: most specific on the left, broader scope to the right. Every parent label must resolve consistently through the operator's chosen naming substrate — a Semantic DNS zone, an LDAP tree, a UNS hierarchy, the operator's own asset registry. **The architecture requires the convention, not any particular DNS server.** Operators with a DNS-based stack get name resolution for free; operators without one still benefit from the predictable shape of the identifiers.

Role labels use ASCII hyphens (`io-master`, `diode-in`), not underscores, for DNS-label compatibility.

**Partition is documented metadata, not part of the FQDN.** Every role has a partition attribute — `acs`, `dmz`, `it`, or `mgmt` — describing which side of the gateway it sits on. Partition is structural; it is not an ISA-95 hierarchy level. The role inventory below carries the partition as a column; deployment records carry it as metadata (alongside class, protocols, hardware-identities, and similar attributes) rather than as a segment of the FQDN.

**Event topics are root-first.** Events the gateway publishes on a broker (`attestation.policy-drift`, `contract.violation`, `security.audit`) follow standard broker topic conventions — root category first, specific event last. They are not FQDNs and they are a separate axis from the role-addressing convention above. Two surfaces, two consumer expectations, two conventions.
...
**Role inventory.** Each row is a role shortname, its purpose, the partition it belongs to (acs / dmz / it / mgmt), the interfaces and inbound conduits it accepts, and the outbound conduits it emits.

| Role | Purpose | Partition | Inbound conduits | Outbound conduits |
|---|---|---|---|---|
| `collect` | Protocol-aware collectors for horizontal (controller↔controller) and vertical (controller↔IO) network traffic — OPC UA, EtherNet/IP, Profinet, Modbus (TCP and RTU), MQTT-SN, fieldbus. Device-level digital interfaces (IO-Link, HART) are observed at the substrate by `io-master`, not at the network by collectors. LoRaWAN / LPWAN ingest via on-box radio or external LoRaWAN gateway publishing MQTT (IO-class data only — never control). | acs | ACS NIC (passive); on-box LoRaWAN radio if equipped | `bus`, `lake` |
| `witness` | Continuous capture, network IDS (signature + contract-attestation observer), scan engine, enrichment | acs | ACS NIC (passive, mirror port) | `lake`, `audit`, `attest` |
| `io-master` | Independent IO substrate observation (IO and industrial Ethernet); cross-checks primary capture | acs | Independent NIC tap / IO interface (separate from `collect`) | `attest` |
| `lake` | Local data lake — durable, append-only, source of truth | acs | `collect`, `witness`, `bus` | `publish` (siphon), `api` (read), `audit` |
| `netservices` | Per-zone IP infrastructure: DHCP scope and lease management, authoritative DNS for the cell (recursive forwarding only when policy allows), NTP server (PTP grandmaster optional), SMB / NFS file and config shares for HMIs and engineering workstations, optional in-cell PKI / certificate authority for zone-internal mTLS. Active participant on the cell network, scoped to the cell, never routed beyond. After Gary Workman, *Two-Box Method* (RTA, 2022). | acs | Zone services interface (active, scoped to the cell) | `audit` (service events: lease/grant/auth records), `lake` (telemetry) |
| `bus` | In-flight message bus; transient, no durable raw data | dmz | `acs.*`, `it.*` | `lake` (write-through), `publish`, `api` |
| `audit` | Audit chain head publisher (full chain on inbound side) | dmz | All zones (write) | `publish` (north replication) |
| `attest` | Attestation aggregator and SDN-policy / catalog correlator; emits `attestation.*` | dmz | `witness`, `io-master`, SDN controller, SPIFFE issuer | `lake`, `audit`, `publish` |
| `publish` | Edge publisher; reads lake, pushes on configured profile | it | `lake`, `bus`, `audit` | IT NIC |
| `api` | Structured query API; mTLS, non-HTTP | it | `lake`, `bus` | IT NIC |
| `tunnel` | Outbound mTLS tunnel agent for the remote-access broker | it | (none — initiates only) | IT NIC (dial-out only) |
| `ui` | Local management UI — **text generator only**, produces candidate configuration artifacts. No privileged access. | mgmt | Management NIC (operator browsers, source-IP-restricted) | `lake` (read-only), `api` (read-only), `cfg` (artifact submission) |
| `cfg` | Configuration parser, validator, staged-state holder, configuration attestation observer | mgmt | `ui` (artifact submission), outbound update channel (artifact pull) | Privileged applier (on-demand, gated), `audit`, `attest` |
...
A connection that fails any active layer is rejected and logged to the audit chain. The attestation observer (`attest`) cross-correlates events from all active layers and emits `attestation.policy-drift` when they disagree. Non-SDN deployments have three layers; the IDS attestation observer compensates by being the only network-plane observer and must be able to alert.

**Cross-side enforcement.** Inbound zones (`acs.*`) and outbound zones (`it.*`) cannot conduit to each other directly. All cross-side traffic transits a DMZ zone (`bus`, `audit`). This is the structural property that makes the boundary auditable — every inbound→outbound message exists as a DMZ event.

## Data Plane

**The local data lake is the source of truth.** All inbound capture, classification output, audit, events, and time-series land in `lake`. The lake is durable, append-only, and the only place raw data persists on the box. Outbound publishers siphon from the lake.

**The in-flight bus (`bus`) is transient.** It exists for low-latency event delivery between zones. It does not retain durable data. Subscribers tail it for live events; consumers expecting historical state read from the lake.

**High fidelity at the edge, downsampled centrally.** The lake stores at native resolution (subsecond, full PCAP segments). Broader-scope BI lakes downstream (open table format on object storage — fed by `publish` when the operator selects that profile) hold downsampled, modeled data for analytics. Full-fidelity slices are pulled on demand from the on-box lake via the structured query API.
...
The kernel firewall conduits, the SPIFFE-based mTLS broker, and the bus subscription policy enforce internal contracts. An attempted connection between workloads not described by a catalog entry is dropped at the firewall and logged as `contract.violation` with source and destination identities, attempted operation, and timestamp.
...
The box catalogs and emits its own observations of contract performance — for both internal and boundary contracts. This data class is the authoritative record of who held up which side, and of every uncontracted communication observed. It is published under `contract.*` and lands in the local lake alongside operational data and the audit chain.

| Telemetry stream | What it records |
|---|---|
| `contract.violation` | Any communication attempt without a corresponding catalog entry — internal (workload-to-workload firewall drops) or external (uncontracted external session). Includes source, destination, attempted operation, timestamp. |
| `contract.connectivity` | Upstream uptime, downtime, latency distribution, throughput, route changes. |
| `contract.delivery` | Per-profile messages sent, acknowledged, retransmitted, dropped, rejected. |
| `contract.auth` | Cert expiry warnings, rotation events, failed authentication attempts (internal mTLS and external). |
| `contract.schema` | Consumer attempted to use deprecated version, schema mismatch detected, contract version pulled by consumer. |
| `contract.quota` | Rate limit hit, payload size exceeded, fair-use violations. |
| `contract.reconciliation` | Sequence-number gaps observed by consumer, watermark queries, replay requests. |
| `contract.audit-verify` | Consumer-side audit chain head validation events; failures here are bilateral (either consumer didn't validate, or chain didn't validate). |
| `contract.sla-breach` | Any of the above that crossed an SLA threshold defined in the contract. |
...
The network IDS in `witness` runs two roles, not one:
...
All attestation findings are emitted under `attestation.*` and route to the same audit chain as security events:

| Stream | Observer | What it records |
|---|---|---|
| `attestation.network` | IDS attestation observer | Contract violations observed on the network plane that did not match the catalog or SDN compilation. |
| `attestation.io` | IO master | IO substrate reading discrepancies between IO master and primary capture, beyond contract tolerance. |
| `attestation.policy-drift` | SDN controller + catalog correlator | SDN policy actually loaded vs catalog-derived policy expected. Firewall rule mismatch with policy compilation. Admission policy mismatch with image trust root. |
| `attestation.identity` | SPIFFE issuer + correlator | SVID issuance events vs contract catalog entries. Identity continuity across rotation. Unknown-identity sessions. |
| `attestation.audit-chain` | Audit chain verifier | On-box audit chain integrity vs replicated chain head at broker. Detects on-box tampering from any peer that has seen a prior head. |
| `attestation.config` | Configuration attestation observer | Running state vs staged configuration artifact. Confirmation events when convergent; divergence events with delta when not (applier bug, partial-apply failure, or out-of-band mutation). |
| `attestation.image` | TPM-bound image attester | TPM-attested running OS-image hash vs intended hash declared by the upstream manager. Bound to measured boot — measurement tampering breaks the TPM signature. Closes the post-update reconciliation loop. |
...
- Records flow into `audit` and are written to an append-only log on the inbound side with a hash chain: each record carries `H(prev_hash || record)`.
- The audit chain head is published every minute via `publish` as `security.audit-head`. The next box up signs it and re-publishes north. Tampering with the on-box log is detectable from any box that has seen a prior chain head.
...
- **Zero inbound on the IT NIC.** The tunnel agent in `tunnel` dials out to the broker over mTLS. The box exposes no remote-access listener on the IT NIC.
...
3. **Submit.** The artifact arrives via the management interface (`ui` → `cfg`) or via the same outbound update channel used for OS images (signed-bundle pull on mTLS).
4. **Verify.** Signature checked. Failure → rejection, audit event, no further processing.
5. **Parse.** The constrained-grammar parser in `cfg` produces a structured intermediate representation. Parse failure → rejection, audit event.
...
A configuration attestation observer cross-checks **running state against the staged configuration artifact**, emitting `attestation.config`:
...
- **Divergent** — running state does not match intended, despite an apparent application. Manager flags this and the box's `attestation.image` and `attestation.config` will already be emitting divergence events.
...
1. Box reports the state it has been holding — current running hashes, accumulated audit chain, accumulated `contract.*` and `attestation.*` adherence telemetry — to upstream.
...
After an update applies, the box's TPM-attested image hash is published as `attestation.image` for the manager to verify. The hash is bound to measured boot — if the box reports an image hash that does not match what is actually running (because of measurement tampering), the TPM signature would not verify. Image attestation closes the loop the same way configuration attestation does: prevention applies, attestation observes, divergence is a flag.

| Attestation stream | What it records |
|---|---|
| `attestation.image` | TPM-attested running image hash vs intended image hash declared by upstream. |
...
7. Box X reboots into the new image; TPM-attested hash published as `attestation.image`.
...
| FR6 | Timely Response to Events | Network IDS with current rule sets; audit chain head publishes every minute; alerts as `security.*`; broker-side runbook automation. |
...
**Zone occupancy across the two units.** The architecture does not change between the single-box realization and the two-box + diode realization. The same role inventory applies; the two units differ only in which zones each one runs.

| Role | Inside box (ACS side) | Outside subscriber (IT side) |
|---|---|---|
| `witness` | ✓ (the substrate is here) | — |
| `collect` | ✓ | — |
| `io-master` | ✓ | — |
| `netservices` | ✓ | — |
| `diode-in` | — | ✓ (receives the diode stream) |
| `lake` | ✓ (the canonical historian) | ✓ (mirror, consumer-readable) |
| `bus` | ✓ | ✓ |
| `audit` | ✓ (audit chain head) | ✓ (replicated, north of the diode) |
| `attest` | ✓ | ✓ |
| `publish` | ✓ (configured for the diode transport) | ✓ (publishes to actual consumers) |
| `api` | — | ✓ |
| `tunnel` | — | ✓ (operator remote-access lands here, not on the inside) |
| `ui` | — | ✓ |
| `cfg` | — | ✓ (config artifacts authored and parsed here; the inside box's config arrives by signed pull) |

The diode itself is a hardware appliance, not an IIA zone. IIA configures `publish` on the inside box for a diode-friendly transport (UDP with FEC; TCP cannot survive a diode) and configures `diode-in` on the outside subscriber to receive that stream.

Net effect: operator access never lands on the inside box. Consumers connect only to the outside subscriber. Compromise of the outside subscriber, or of any consumer, cannot reach the inside — physics, not policy, prevents the return path.
...
| Zone IP services (DHCP / DNS / NTP / file / PKI) | dnsmasq (DHCP + DNS combined, small footprint), ISC Kea (DHCP), BIND or NSD (authoritative DNS), Unbound (recursive DNS), chrony or ntpd (time), Samba (SMB), nfs-kernel-server (NFS), step-ca or smallstep (in-cell PKI) | Operator selects per zone size and existing skill base. dnsmasq + chrony + Samba is a small-footprint default that fits the appliance SKU; larger deployments often split DHCP and DNS onto separate daemons for operational separation. |
