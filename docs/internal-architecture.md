...
| `ot.acs.netservices` | Per-zone IP infrastructure: DHCP scope and lease management, authoritative DNS for the cell (recursive forwarding only when policy allows), NTP server (PTP grandmaster optional), SMB / NFS file and config shares for HMIs and engineering workstations, optional in-cell PKI / certificate authority for zone-internal mTLS. Active participant on the cell network, scoped to the cell, never routed beyond. After Gary Workman, *Two-Box Method* (RTA, 2022). | Inbound | Zone services interface (active, scoped to the cell) | `ot.dmz.audit` (service events: lease/grant/auth records), `ot.acs.lake` (telemetry) |
